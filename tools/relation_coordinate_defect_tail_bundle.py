"""Immutable, closed-inventory packaging of the relation-coordinate certification lab."""
import argparse
import gzip
import io
import json
...
import defect_recurrence_tail_bundle as parent_bundle
from relation_coordinate_defect_tail_audit import audit, PARENT, DECISION

PARENT_MANIFEST='3864806e6332d0d6fcf554bb73bc1cec447f451ac0ae456d99041b0e5dcb74d1'
...
def check(root,repo=None):
    ...
    assert manifest['selected_precision']==96 and manifest['promotion']=='NO_PROMOTION'
    ...
    for old in (root/'parent/source').rglob('*'):
        ...
        if str(relative)=='formal/MLSFormal/AxiomReport.lean':
            retained=[s for s in new.read_text().splitlines() if s.strip() and 'relationTail_' not in s
                      and s!='import MLSFormal.RelationCoordinateDefectTailCertification']
...
def build(repo,parent,work,output):
    ...
    manifest=dict(schema='mls.relation-coordinate-defect.manifest.v1',source_sha=sha,source_tree=tree,
                  parent_sha=PARENT,decision=DECISION,selected_precision=96,promotion='NO_PROMOTION',
                  files=files(output))
    (output/'manifest.json').write_bytes(encode(manifest))
    (output/'outer-seal.json').write_bytes(encode(dict(schema='mls.relation-coordinate-defect.outer.v1',
