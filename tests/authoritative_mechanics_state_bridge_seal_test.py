#!/usr/bin/env python3
"""Outer-seal deterministic positive and mutation regression."""
...
GROUPS = (
    "raw-a", "raw-b", "oracle", "parent-relation-geometry", "source",
    "receipts", "docs",
)
...
def main() -> int:
    ...
    parser.add_argument(
        "--tool", type=Path,
        default=root / "tools" / "seal_authoritative_mechanics_state_bridge_evidence.py",
    )
    arguments = parser.parse_args()
    with tempfile.TemporaryDirectory(prefix="mls-mechanics-bridge-seal-") as temporary:
    ...
    print("authoritative mechanics bridge outer-seal regression: PASS (2 positives, 3 mutations)")
    return 0
