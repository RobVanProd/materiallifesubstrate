#!/usr/bin/env python3
"""Outer-seal positive and mutation regression for time-integration evidence."""
...
GROUPS = (
    "raw-a", "raw-b", "oracle", "parent-drift-bridge", "source",
    "receipts", "docs",
)
...
def main() -> int:
    ...
    parser.add_argument(
        "--tool", type=Path,
        default=root / "tools" / "seal_time_integration_foundation_evidence.py",
    )
    arguments = parser.parse_args()
    with tempfile.TemporaryDirectory(prefix="mls-time-integration-seal-") as temporary:
        bundle = fixture(Path(temporary))
        created = invoke(
            arguments.tool, "create", "--bundle", str(bundle), "--source-sha",
            "0123456789abcdef0123456789abcdef01234567", "--ci-run-id", "1",
        )
        ...
        value["decision"] = "retain_quantized_stormer_verlet_dynamics_candidate_for_research"
    ...
    print("time integration outer-seal regression: PASS (2 positives, 3 mutations)")
    return 0
