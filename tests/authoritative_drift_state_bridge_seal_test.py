#!/usr/bin/env python3
"""Outer-seal deterministic positive and mutation regression for drift evidence."""
...
GROUPS = (
    "raw-a", "raw-b", "oracle", "parent-mechanics-bridge", "source",
    "receipts", "docs",
)
...
def main() -> int:
    ...
    parser.add_argument(
        "--tool", type=Path,
        default=root / "tools" / "seal_authoritative_drift_state_bridge_evidence.py",
    )
    arguments = parser.parse_args()
    with tempfile.TemporaryDirectory(prefix="mls-drift-bridge-seal-") as temporary:
    ...
    print("authoritative drift bridge outer-seal regression: PASS (2 positives, 3 mutations)")
    return 0
