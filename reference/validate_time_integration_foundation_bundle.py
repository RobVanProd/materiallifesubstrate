#!/usr/bin/env python3
"""Fresh validator for sealed Time Integration Foundation evidence."""
...
WORKFLOW = "Time Integration Foundation Lab"
...
def invoke(command: list[str], cwd: Path, timeout: int = 1800) -> subprocess.CompletedProcess[str]:
...
def validate_source_identity(bundle: Path, source_sha: str) -> None:
    ...
    expected_fields = {
        "schema", "repository", "branch", "source_sha", "tree_sha",
    ...
    if (
        identity["schema"] != "mls.time-integration-foundation.source.v1"
    ...
    archive = bundle / "source" / identity["archive"]
    if (
        not archive.is_file()
...
def validate_ci_receipt(bundle: Path, source_sha: str, run_id: int) -> None:
    ...
    expected_fields = {
        "schema", "repository", "workflow", "run_id", "run_attempt",
        "head_sha", "head_branch", "event", "conclusion", "jobs",
    }
    if set(receipt) != expected_fields:
        raise RuntimeError("CI receipt field inventory differs")
    if (
        receipt["schema"] != "mls.time-integration-foundation.ci.v1"
...
def main() -> int:
    ...
    try:
        seal = invoke(
            [sys.executable,
             str(source / "tools" / "seal_time_integration_foundation_evidence.py"),
        ...
        with tempfile.TemporaryDirectory(prefix="mls-time-integration-verify-") as temporary:
            output = Path(temporary) / "oracle-summary.json"
            oracle = invoke(
                [sys.executable,
                 str(source / "reference" / "time_integration_foundation_oracle.py"),
                 "--raw", str(bundle / "raw-a"), "--output", str(output)],
                source,
            )
            ...
            if (
                ...
                or summary.get("exact_gates", {}).get("reversible") is not True
                or summary.get("exact_gates", {}).get("domain_atomic") is not True
            ):
                raise RuntimeError("fresh scientific disposition differs")
        parent = invoke(
            [sys.executable,
             str(source / "reference" / "validate_authoritative_drift_state_bridge_bundle.py"),
             "--bundle", str(bundle / "parent-drift-bridge"),
             "--source", str(source)],
            source,
        )
        require_success(parent, "accepted parent drift evidence")
        if arguments.run_mutations:
            mutations = invoke(
                [sys.executable,
                 str(source / "tests" / "time_integration_foundation_oracle_test.py"),
                 "--raw", str(bundle / "raw-a")], source,
            )
            require_success(mutations, "oracle mutation regression")
        print(
            "TIME INTEGRATION FOUNDATION EVIDENCE VALID: "
        ...
        print(f"TIME INTEGRATION FOUNDATION EVIDENCE INVALID: {error}", file=sys.stderr)
        return 1
