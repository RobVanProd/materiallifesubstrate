#!/usr/bin/env python3
"""Fresh validator for sealed Authoritative Drift State Bridge evidence."""
...
def main() -> int:
    ...
    try:
        seal = invoke(
            [sys.executable,
             str(source / "tools" / "seal_authoritative_drift_state_bridge_evidence.py"),
             "verify", "--bundle", str(bundle)],
            source,
        )
        require_success(seal, "outer seal")
        compare_directories(bundle / "raw-a", bundle / "raw-b")
        with tempfile.TemporaryDirectory(prefix="mls-drift-bridge-verify-") as temporary:
            output = Path(temporary) / "oracle-summary.json"
            oracle = invoke(
                [sys.executable,
                 str(source / "reference" / "authoritative_drift_state_bridge_oracle.py"),
            ...
            if output.with_suffix(".csv").read_bytes() != (
                bundle / "oracle" / "oracle-summary.csv"
            ).read_bytes():
            ...
            if (
                ...
                or summary.get("selected_refinement") != 128
                or summary.get("parent_fingerprint_reproduced") is not True
                or summary.get("inherited_impulse_and_kinetic_gates") is not True
                or summary.get("explicit_remainder_evaluated") is not False
        ...
        if arguments.run_mutations:
            mutations = invoke(
                [sys.executable,
                 str(source / "tests" / "authoritative_drift_state_bridge_oracle_test.py"),
                 "--raw", str(bundle / "raw-a")],
                source,
            )
        ...
        print(
            "AUTHORITATIVE DRIFT STATE BRIDGE EVIDENCE VALID: "
            f"source={seal_value['source_sha']} files={len(seal_value['payload'])} "
            f"prehash={seal_value['outer_pre_hash']} R=128 NO_PROMOTION"
        )
        return 0
    except (OSError, ValueError, RuntimeError, json.JSONDecodeError) as error:
        print(f"AUTHORITATIVE DRIFT STATE BRIDGE EVIDENCE INVALID: {error}", file=sys.stderr)
        return 1
